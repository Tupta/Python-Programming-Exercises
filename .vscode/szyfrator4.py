...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        # utwórz pole wyboru szyfru 'BITWAOCHMURY'
        self.BITWAOCHMURY = BooleanVar()
        Checkbutton(self,
                    text="BITWAOCHMURY",
                    variable=self.BITWAOCHMURY
    ...
    def custom_replace(self, text, selected_cipher):
        # Zdefiniuj zestawy zastąpień liter dla różnych szyfrów
        ciphers = {
            "GADERYPOLUKI": {
            'g': 'a',
            'a': 'g',
        ...
        result = ""
        if selected_cipher in ciphers:
            cipher = ciphers[selected_cipher]
            for char in text:
                if char in cipher:
                    result += cipher[char]
                else:
    ...
    def encryptmessage(self):
        ...
        input_text = self.person_ent.get().lower()

        #sprawdź który szyfr został wybrany
        selected_cipher = ""
        if self.GADERYPOLUKI.get():
            selected_cipher = "GADERYPOLUKI"
        elif self.MALINOWEBUTY.get():
            selected_cipher = "MALINOWEBUTY"
        elif self.BITWAOCHMURY.get():
            selected_cipher = "BITWAOCHMURY"
        
        # Wykonaj niestandardową zamianę liter
        encrypted_message = self.custom_replace(input_text, selected_cipher)
