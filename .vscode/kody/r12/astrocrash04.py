...
class Missile(games.Sprite):
    """Pocisk wystrzelony przez statek"""
    image = games.load_image("pocisk.bmp")
    sound = games.load_sound("pocisk.wav")
    BUFFER = 40  # STAŁA DEFINIUJE ODLEGŁOŚĆ MIEJSCA UTWORZENIA pocisku od statku
    VELOCITY_FACTOR = 7 # stała określa szybkość lotu pocisku
    LIFETIME = 40 # stąła okreśła czas życia pocisku
    
    #jak każda kalsa i ta musi mieć swojego konstruktora
    
    def __init__(self, ship_x, ship_y, ship_angle):
        Missile.sound.play()
        
        #zeby posick lecial zgodnie z kierunkiem statku trzeba uzyc konta statku w danym momencie i zamienic go na radiany
        angle = ship_angle *math.pi/180
        
        #obliczenie pozycji poczxątkowej pocisku
        buffer_x = Missile.BUFFER * math.sin(angle)
        buffer_y = Missile.BUFFER * -math.cos(angle)
        x = ship_x + buffer_x
        y = ship_y + buffer_y
        
        #składowe predkosci pocisku
